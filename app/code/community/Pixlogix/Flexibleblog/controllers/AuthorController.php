...
class Pixlogix_Flexibleblog_AuthorController extends Mage_Core_Controller_Front_Action
{
     ...
     * index Action
     *
     * @return author wise post list page
     */
        ...
        if( Mage::helper('flexibleblog')->enableBreadcrumbs() )
	{
            ...
            $breadcrumbs->addCrumb('author',
                array(
                    'label' => Mage::helper('flexibleblog')->__($author_name),
                    'title' => Mage::helper('flexibleblog')->__($author_name)
                )
            );
        }
        $this->renderLayout();
